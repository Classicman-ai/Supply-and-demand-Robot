...
class EntryEngine:
    ...
    def evaluate(self, analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        validation = analysis.get("zone_validation_by_id", {}).get(zone.zone_id)
        if not isinstance(validation, dict):
            return self._reject("ZONE_VALIDATION_REQUIRED", ["selected real zone has no ZoneValidationEngine result"])
        if not bool(validation.get("validated")):
            return self._reject(
                "ZONE_VALIDATION_REJECTED",
                list(validation.get("warnings", [])) or list(validation.get("reasons", [])) or ["selected zone did not meet the real validation threshold"],
            )
        ...
        return {
            ...
            "zone_grade": validation.get("grade"),
            "zone_score": validation.get("score"),
            "zone_validation": validation,
            "reason": f"{zone.zone_type.value} zone with M15/M5 confirmation and real zone validation",
